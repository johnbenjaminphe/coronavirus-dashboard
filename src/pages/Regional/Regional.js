...
const Regional: ComponentType<Props> = ({}: Props) => {
  ...
  return (
    <Styles.Container className="govuk-width-container">
      ...
      {view === 'chart' && (
        <>
          <LineChart data={overviewData?.K02000001?.dailyTotalConfirmedCases ?? []} header="Total number of lab-confirmed UK cases" tooltipText="cases" />
          <BarChart data={overviewData?.K02000001?.dailyConfirmedCases ?? []} header="Daily number of lab-confirmed cases" tooltipText="cases" />
          <LineChart data={overviewData ?.K02000001 ?.dailyTotalDeaths ?? []} header="Total number of COVID-19 associated UK deaths in hospital" tooltipText="deaths" />
          <BarChart data={overviewData?.K02000001?.dailyDeaths ?? []} header="Daily number of COVID-19 associated UK deaths in hospital" tooltipText="deaths" />
        </>
      )}
      {view === 'table' && (
        <>
